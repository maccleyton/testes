...
function setTemperature(tempC, applyVisual = true) {
  ...
  applyThermalParams();              // única fonte de velocidade térmica
  if (applyVisual) { isTemperatureApplied = true; applyTemperature(); }
}
...
// Presets
document.querySelectorAll('.preset-btn').forEach(btn => {
  btn.addEventListener('click', () => setTemperature(parseFloat(btn.dataset.temp), true));
});
